...
def training(train_dataset, test_dataset, input_shape, lr_init=0.01, use_mse_loss=True, batch_size=32, reg_factor=5e-4,
    ...
    history = Model_KD.fit(ds_train, epochs=epochs - start_epoch + 1, verbose=1,
                 steps_per_epoch=(len(train_dataset)*4 // batch_size),
                #  steps_per_epoch=10,
                 validation_data=ds_test,
                 validation_steps=len(test_dataset)*4 // (batch_size),
                 callbacks=callback
                 )
    # Convert the history dictionary to a dataframe
    history_df = pd.DataFrame(history.history)
    # Save the dataframe to a CSV file
    history_df.to_csv('kda_history_UR.csv', index=False)
...
def predic(test_dataset, input_shape, save_path, use_attention, drop_rate=0.5, lr_init = 0.01):
    AUTOTUNE = tf.data.experimental.AUTOTUNE
    y_test=[]
    ...
    np.savez('y_predict_kda_ur.npz', data=y_predict)
    ...
    disp.plot()
    plt.savefig('Confusion_matrix_kda_ur.png')
    plt.show()

def cm_cal():
    y_predict = np.load('y_predict_kda_ur.npz')['data']
...
def main():
    ...
    predic(test_dataset, input_shape=input_shape, save_path=save_path, use_attention = use_attention, drop_rate=drop_rate, lr_init = 0.01)
    # cm_cal()
