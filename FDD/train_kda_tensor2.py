...
from gen_tensor2 import get_data, generator_training_data, data_augmentation, generator_test_data
...
def training(train_dataset, test_dataset, input_shape, lr_init=0.01, use_mse_loss=True, batch_size=32, reg_factor=5e-4,
    ...
    if start_epoch > 1:
        ...
        path1 = os.path.join(save_path, 'checkpoints', 'kda_fdd_t_epoch_' + str(start_epoch) + '.h5')
        resnet_50.load_weights(path1)
        path2 = os.path.join(save_path, 'checkpoints', 'kda_fdd_s_epoch_' + str(start_epoch) + '.h5')
    ...
    history = Model_KD.fit(ds_train, epochs=epochs - start_epoch + 1, verbose=1,
                ...
                #  steps_per_epoch=10,
                 ...
                 validation_steps=len(test_dataset)*2 // (batch_size),
                 callbacks=callback
                 )
    # Convert the history dictionary to a dataframe
    history_df = pd.DataFrame(history.history)
    # Save the dataframe to a CSV file
    history_df.to_csv('kda_fdd_history.csv', index=False)
...
def testing(test_dataset, args, save_path='save_model', lr_init=0.01, use_attention = False, drop_rate=0.5):
    ...
    file_weight = os.path.join(save_path, "kda_fdd_best_s_model.h5")
...
def predic(test_dataset, input_shape, save_path, use_attention, drop_rate, lr_init = 0.01):
    AUTOTUNE = tf.data.experimental.AUTOTUNE
    y_test=[]
    ...
    np.savez('y_predict.npz', data=y_predict)
...
def cm_cal():
    y_predict = np.load('y_predict.npz')['data']
...
def main():
    ...
    predic(test_dataset, input_shape=input_shape, save_path=save_path, lr_init=lr_init, use_attention = use_attention, drop_rate=drop_rate)
    # cm_cal()
