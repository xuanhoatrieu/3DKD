...
class EpochCheckpoint(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if (self.intEpoch) % self.every == 0:
            ...
            p1 = os.path.sep.join([self.outputPath, "kda_fdd_t_epoch_{}.h5".format(self.intEpoch)])
            self.model.teacher_model.save_weights(p1, overwrite=True)
            p2 = os.path.sep.join([self.outputPath, "kda_fdd_s_epoch_{}.h5".format(self.intEpoch)])
            self.model.student_model.save_weights(p2, overwrite=True)

            # Delete old model weight
            old_p1 = os.path.sep.join([self.outputPath, "kda_fdd_s_epoch_{}.h5".format(self.intEpoch - self.every)])
            old_p2 = os.path.sep.join([self.outputPath, "kda_fdd_t_epoch_{}.h5".format(self.intEpoch - self.every)])
...
class ModelCheckpoint(BaseLogger):
    def __init__(self, folderpath, jsonPath=None, jsonName=None, startAt=0, monitor='val_accuracy', mode='max',
        ...
        self.filepath_t = os.path.join(folderpath, 'kda_fdd_best_t_model.h5')
        self.filepath_s = os.path.join(folderpath, 'kda_fdd_best_s_model.h5')
